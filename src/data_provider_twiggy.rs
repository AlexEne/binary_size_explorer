    ...
    pub fn from_path<P: AsRef<std::path::Path>>(arena: &'a Arena, path: P) -> Self {
        ...
        for item in items.iter() {
            ...
            let (locals, function_ops) = match item.kind() {
                ...
                twiggy_ir::ItemKind::Data(_data) => (Array::new(arena, 0), Array::new(arena, 0)),
                twiggy_ir::ItemKind::Debug(_debug_info) => {
                    (Array::new(arena, 0), Array::new(arena, 0))
                }
                twiggy_ir::ItemKind::Misc(_misc) => (Array::new(arena, 0), Array::new(arena, 0)),
            };
            ...
            raw_data.push(FunctionData {
                function_property: FunctionProperty {
                    raw_name: String::from_str(arena, name),
                    demangled_name: Some(String::from_str(arena, name)),
                    monomorphization_of: monomorphization_of.map(|m| String::from_str(arena, m)),
                    shallow_size_bytes,
...
fn get_locals_and_ops_for_function<'a, 'b>(
    ...
    data: &'b [u8],
    range: &'b Range<usize>,
) -> (Array<'a, String<'a>>, Array<'a, FunctionOp<'a>>) {
    let function_body =
    ...
    while let Ok((op, offset)) = body.read_with_offset() {
        ...
        temp_ops.push(FunctionOp::new(addr as u64, String::new(arena, 0)));
    }
    ...
    // let mut ops2 = Array::new(arena, count);
    // for op in ops {
    //     ops2.push(op);
    // }
